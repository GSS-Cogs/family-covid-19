...
if (tab_name_expected in tabs) != True:
    ...
    if expected_title in title_of_tab:
        ...
        admin_port = ref_cell.shift(1,0).expand(DOWN)#.is_not_blank() #to be changed to ONS Geography Code (9 Digit code), ignoring unkown rows 
        vessel_length = 'All'
        species_group = 'All'
        unit = '' #Will be changed later once measure time has been defined. 
        measure_type = ref_cell.shift(1,-3).expand(RIGHT).is_not_blank()
        observations = period.fill(DOWN).is_not_blank() - ref_cell.shift(1,1).expand(RIGHT) - tab.excel_ref('A46').expand(RIGHT).expand(DOWN)
        #savepreviewhtml(admin_port)
        dimensions = [
            HDim(period, 'Period', CLOSEST, LEFT),
            HDim(country, 'Country', CLOSEST, ABOVE),
            HDim(admin_port, 'Admin Port', DIRECTLY, LEFT),
            HDim(measure_type, 'Measure Type', CLOSEST, LEFT),
            HDimConst('Vessel Length', vessel_length),
            HDimConst('Species Group', species_group),
            HDimConst('Unit', unit),
        ]
...
df = df.replace({'Measure Type' : { "Value (£000's)" : 'GBP Thousands', 'Quantity (tonnes)' : 'Weight', 'Number of trips' : 'Count' }})
df = df.replace({'Admin Port' : { "" : 'UK total'}})
