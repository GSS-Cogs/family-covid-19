...
if (tab_name_expected in tabs) != True:
    ...
    if expected_title in title_of_tab:
        ...
        species_group = ref_cell.expand(DOWN).is_not_blank()  #to be changed to ONS Geography Code (9 Digit code)
        country = ref_cell.shift(1,0).expand(DOWN)#.is_not_blank() #to be changed to ONS Geography Code (9 Digit code), ignoring unkown rows 
        vessel_length = 'All'
        ...
        dimensions = [
            ...
            HDim(species_group, 'Species Group', CLOSEST, ABOVE),
...
df = df.replace({'Species Group' : { "UK total" : 'All'}})
