...
for tab in tabs:
    if tab.name in ['Table 4']:
        columns=["Period", "Cause of detah", "Sex", "Quintile", "Country", "Rate", "Lower CI", "Upper CI", 'Marker', "Measure type", "Unit"]
        ...
        quintile = tab.filter(contains_string('Quintile')).shift(0,1).expand(DOWN).is_not_blank() - tab.filter(contains_string('Footnotes')).expand(DOWN)
        trace.Quintile('Quintile given at cell range: {}', var = excelRange(quintile))

        country = "Wales"
        trace.Country("Country hardcoded as Wales")
...
tidy_4 = df[['Period', 'Cause of death', 'Sex', 'Country', 'Quintile', 'Rate', 'Lower CI', 'Upper CI', 'Value', 'Measure Type', 'Unit']]

out = Path('out')
out.mkdir(exist_ok=True)
title = pathify(datasetTitle)
output_title = title + "Number_of_deaths_and_age-standardised_rates_by sex,_deprivation_quintiles_in_Wales"
scraper.dataset.comment = notes
tidy_4.drop_duplicates().to_csv(out / f'{output_title}.csv', index = False)
