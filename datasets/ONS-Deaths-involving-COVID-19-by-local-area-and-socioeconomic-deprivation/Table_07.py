...
for tab in tabs:
    if tab.name in ['Table 7']:
        columns=["Period", "Cause of detah", "Major Towns and Cities", "Country", "Rate", "Lower CI", "Upper CI", 'Marker', "Measure type", "Unit"]
        ...
        town_city = tab.filter(contains_string('Major Towns and Cities')).shift(0,1).expand(DOWN).is_not_blank() 
        trace.Major_Towns_and_Cities('Major Towns and Cities given at cell range: {}', var = excelRange(town_city))
        ...
        dimensions = [
            ...
            HDim(town_city, 'Major Towns and Cities', DIRECTLY, LEFT),
...
tidy_7 = df[['Period', 'Cause of death', 'Country', 'Major Towns and Cities', 'Rate', 'Lower CI', 'Upper CI', 'Value', 'Measure Type', 'Unit']]

out = Path('out')
out.mkdir(exist_ok=True)
title = pathify(datasetTitle)
output_title = title + "_Number_of_deaths_and_age-standardised_rates_by_Major_Towns_and_Cities_in_England_and_Wales,"
scraper.dataset.comment = notes
tidy_7.drop_duplicates().to_csv(out / f'{output_title}.csv', index = False)
