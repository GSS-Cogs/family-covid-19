...
for tab in tabs:
    if tab.name in ['Table 8']:
        columns=["Period", "Cause of detah", "Travel To Work Areas", "Country", "Rate", "Lower CI", "Upper CI", 'Marker', "Measure type", "Unit"]
        ...
        travel_work_area = tab.filter(contains_string('Travel To Work Areas')).shift(0,1).expand(DOWN).is_not_blank() - cause_of_death
        trace.Travel_To_Work_Areas('Travel To Work Areas given at cell range: {}', var = excelRange(travel_work_area))
        ...
        dimensions = [
            ...
            HDim(travel_work_area, 'Travel To Work Areas', DIRECTLY, LEFT),
...
9 : = not available The age-standardised rate and its lower and upper confidence interval is unavailable.
"""
...
tidy_8 = df[['Period', 'Cause of death', 'Country', 'Travel To Work Areas', 'Rate', 'Lower CI', 'Upper CI', 'Value', 'Measure Type', 'Unit']]

out = Path('out')
out.mkdir(exist_ok=True)
title = pathify(datasetTitle)
output_title = title + "_Number_of_deaths_and_age-standardised_rates_by_Travel_To_Work_Areas_in_England_and_Wales,"
scraper.dataset.comment = notes
tidy_8.drop_duplicates().to_csv(out / f'{output_title}.csv', index = False)
