...
try : 
    ...
    observations = authority.fill(DOWN).is_not_blank().is_not_whitespace() 
    Dimensions = [
...
tidy = next_table[['Period','Local Authority Settings', 'Measure Type','Unit',  'Value', 'Marker']]

out = Path('out')
out.mkdir(exist_ok=True)
tidy.drop_duplicates().to_csv(out / 'Attendance at local authority settings during the coronavirus COVID-19 pandemic.csv', index = False)
